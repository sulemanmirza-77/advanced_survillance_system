...
const CurrentVideoPage = () => {
    ...
    if (!user) {
      ...
      try {
        const response = await axios.put(
          `http://localhost:5500/api/user/sub/${currentVideo?.userId?._id}`,
          null,
          {
            headers: {
  ...
  const handleUnsubscribe = async () => {
    if (!user) {
      ...
      try {
        const response = await axios.put(
          `http://localhost:5500/api/user/unSub/${currentVideo?.userId?._id}`,
          null,
          {
            headers: {
              Authorization: `Bearer ${user?.access_token}`,
  ...
  const handleLike = async () => {
    if (!user) {
      ...
      try {
        await axios.put(
          `http://localhost:5500/api/video/like/${currentVideo?._id}`,
          null,
          {
            headers: {
              Authorization: `Bearer ${user?.access_token}`,
  ...
  const handleDislike = async () => {
    if (!user) {
      ...
      try {
        await axios.put(
          `http://localhost:5500/api/video/dislike/${currentVideo?._id}`,
          null,
          {
            headers: {
              Authorization: `Bearer ${user?.access_token}`,
  ...
  const handleDelete = async () => {
    try {
      const response = await axios.delete(
        `http://localhost:5500/api/video/delete/${currentVideo?._id}`,
        {
          headers: {
            Authorization: `Bearer ${user?.access_token}`,
          },
